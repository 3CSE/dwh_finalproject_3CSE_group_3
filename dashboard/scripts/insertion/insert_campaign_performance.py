...
def get_card_by_name(session_token, name):
    ...
    for card in cards:
        if card['name'] == name:
            return card['id']
    raise Exception(f"Card '{name}' not found. Please run campaign_performance_cards.py first.")

def build_campaign_performance_dashboard():
    """Add Campaign Performance cards to the Campaign Performance Dashboard"""
    logging.info("Adding Campaign Performance cards to dashboard...")
    ...
    dashboard_id = get_dashboard_by_name(session_token, "Campaign Performance")
    logging.info(f"✓ Found Campaign Performance Dashboard (ID: {dashboard_id})")
    
    # Define card names and their positions on the dashboard
    card_layout = [
        # Row 1: KPI Cards (4 cards, 6 columns each = 24 total)
        {
        ...
        # Row 2: Campaign Performance Trends (full width = 24 columns)
        {
            "name": "Campaign Performance Trends",
            "position": {"row": 4, "col": 0, "size_x": 24, "size_y": 6}
        },
        # Row 3: Scatter Plot + Row Charts (8 columns each = 24 total)
        {
            "name": "Discount vs Revenue Analysis",
            "position": {"row": 10, "col": 0, "size_x": 8, "size_y": 8}
        },
        {
            "name": "Top 5 Campaigns by Revenue",
            "position": {"row": 10, "col": 8, "size_x": 8, "size_y": 8}
        },
        {
            "name": "Top 5 Campaigns by Order Count",
            "position": {"row": 10, "col": 16, "size_x": 8, "size_y": 8}
        }
    ]
    ...
    if missing_cards:
        ...
        logging.error("  python dashboard/scripts/cards/campaign_performance_cards.py")
    ...
    logging.info(f"\n✅ Campaign Performance dashboard populated successfully!")
    logging.info(f"View at: {METABASE_URL}/dashboard/{dashboard_id}")
    return dashboard_id

if __name__ == "__main__":
    try:
        build_campaign_performance_dashboard()
    except Exception as e:
        logging.error(f"❌ Failed to populate Campaign Performance dashboard: {e}")
        exit(1)
