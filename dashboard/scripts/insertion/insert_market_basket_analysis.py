...
def insert_market_basket_analysis_cards():
    logging.info("Inserting Market Basket Analysis cards...")
    session_token = login_to_metabase()
    dashboard_id = get_dashboard_by_name(session_token, "Market Basket Analysis")
    
    card_layout = [
        {"name": "Average Items per Order", "position": {"row": 0, "col": 0, "size_x": 6, "size_y": 4}},
        {"name": "Multi-Item Orders Percentage", "position": {"row": 0, "col": 6, "size_x": 6, "size_y": 4}},
        {"name": "Cross-Sell Rate", "position": {"row": 0, "col": 12, "size_x": 6, "size_y": 4}},
        {"name": "Average Basket Value", "position": {"row": 0, "col": 18, "size_x": 6, "size_y": 4}},
        {"name": "Product Pair Associations", "position": {"row": 4, "col": 0, "size_x": 12, "size_y": 8}},
        {"name": "Product Association Matrix", "position": {"row": 4, "col": 12, "size_x": 12, "size_y": 8}},
        {"name": "Basket Size Distribution", "position": {"row": 12, "col": 0, "size_x": 24, "size_y": 6}},
    ]
    ...
    response.raise_for_status()
    logging.info(f"✅ Market Basket Analysis dashboard populated! View at: {METABASE_URL}/dashboard/{dashboard_id}")
    return dashboard_id

if __name__ == "__main__":
    insert_market_basket_analysis_cards()
