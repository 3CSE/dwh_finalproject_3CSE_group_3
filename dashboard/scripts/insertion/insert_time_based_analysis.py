...
def insert_time_based_analysis_cards():
    logging.info("Inserting Time-Based Analysis cards...")
    session_token = login_to_metabase()
    dashboard_id = get_dashboard_by_name(session_token, "Time-Based Analysis")
    
    card_layout = [
        {"name": "Year-over-Year Growth", "position": {"row": 0, "col": 0, "size_x": 6, "size_y": 4}},
    ...
    response.raise_for_status()
    logging.info(f"✅ Time-Based Analysis dashboard populated! View at: {METABASE_URL}/dashboard/{dashboard_id}")
    return dashboard_id

if __name__ == "__main__":
    insert_time_based_analysis_cards()
