...
def insert_geographic_performance_cards():
    logging.info("Inserting Geographic Performance cards...")
    session_token = login_to_metabase()
    dashboard_id = get_dashboard_by_name(session_token, "Geographic Performance")
    
    card_layout = [
        {"name": "Total Countries", "position": {"row": 0, "col": 0, "size_x": 6, "size_y": 4}},
    ...
    response.raise_for_status()
    logging.info(f"✅ Geographic Performance dashboard populated! View at: {METABASE_URL}/dashboard/{dashboard_id}")
    return dashboard_id

if __name__ == "__main__":
    insert_geographic_performance_cards()
