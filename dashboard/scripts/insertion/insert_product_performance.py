...
def insert_product_performance_cards():
    logging.info("Inserting Product Performance cards...")
    session_token = login_to_metabase()
    dashboard_id = get_dashboard_by_name(session_token, "Product Performance")
    
    card_layout = [
        {"name": "Total Products", "position": {"row": 0, "col": 0, "size_x": 6, "size_y": 4}},
    ...
    response.raise_for_status()
    logging.info(f"✅ Product Performance dashboard populated! View at: {METABASE_URL}/dashboard/{dashboard_id}")
    return dashboard_id

if __name__ == "__main__":
    insert_product_performance_cards()
