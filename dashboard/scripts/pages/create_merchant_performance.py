...
def create_merchant_performance_dashboard():
    logging.info("Creating Merchant Performance Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Merchant Performance")
    if existing_id:
        logging.info(f"✓ Merchant Performance Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Merchant Performance", "Merchant activity, sales, and performance metrics")
    logging.info(f"✅ Merchant Performance Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_merchant_performance_dashboard()
