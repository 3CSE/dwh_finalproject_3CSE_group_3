...
def create_market_basket_analysis_dashboard():
    logging.info("Creating Market Basket Analysis Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Market Basket Analysis")
    if existing_id:
        logging.info(f"✓ Market Basket Analysis Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Market Basket Analysis", "Product associations, cross-selling opportunities, and bundle insights")
    logging.info(f"✅ Market Basket Analysis Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_market_basket_analysis_dashboard()
