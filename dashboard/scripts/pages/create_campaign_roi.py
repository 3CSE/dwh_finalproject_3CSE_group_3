...
def create_campaign_roi_dashboard():
    logging.info("Creating Campaign ROI & Effectiveness Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Campaign ROI & Effectiveness")
    if existing_id:
        logging.info(f"✓ Campaign ROI & Effectiveness Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Campaign ROI & Effectiveness", "Detailed campaign return on investment and effectiveness metrics")
    logging.info(f"✅ Campaign ROI & Effectiveness Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_campaign_roi_dashboard()
