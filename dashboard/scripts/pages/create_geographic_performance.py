...
def create_geographic_performance_dashboard():
    logging.info("Creating Geographic Performance Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Geographic Performance")
    if existing_id:
        logging.info(f"✓ Geographic Performance Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Geographic Performance", "Regional sales performance and geographic distribution insights")
    logging.info(f"✅ Geographic Performance Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_geographic_performance_dashboard()
