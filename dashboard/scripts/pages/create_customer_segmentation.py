...
def create_customer_segmentation_dashboard():
    logging.info("Creating Customer Segmentation Deep Dive Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Customer Segmentation Deep Dive")
    if existing_id:
        logging.info(f"✓ Customer Segmentation Deep Dive Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Customer Segmentation Deep Dive", "Detailed customer segments, cohort analysis, and behavioral clustering")
    logging.info(f"✅ Customer Segmentation Deep Dive Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_customer_segmentation_dashboard()
