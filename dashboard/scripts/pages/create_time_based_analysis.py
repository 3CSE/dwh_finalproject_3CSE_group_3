...
def create_time_based_analysis_dashboard():
    logging.info("Creating Time-Based Analysis Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Time-Based Analysis")
    if existing_id:
        logging.info(f"✓ Time-Based Analysis Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Time-Based Analysis", "Temporal patterns, trends, and time-series analytics")
    logging.info(f"✅ Time-Based Analysis Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_time_based_analysis_dashboard()
