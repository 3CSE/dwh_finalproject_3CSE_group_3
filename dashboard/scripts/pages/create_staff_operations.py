...
def create_staff_operations_dashboard():
    logging.info("Creating Staff & Operations Dashboard...")
    session_token = login_to_metabase()
    existing_id = get_dashboard_by_name(session_token, "Staff & Operations")
    if existing_id:
        logging.info(f"✓ Staff & Operations Dashboard already exists (ID: {existing_id})")
        return existing_id
    dashboard_id = create_dashboard(session_token, "Staff & Operations", "Staff productivity, operations metrics, and workforce analytics")
    logging.info(f"✅ Staff & Operations Dashboard created (ID: {dashboard_id})")
    return dashboard_id

if __name__ == "__main__":
    try:
        create_staff_operations_dashboard()
